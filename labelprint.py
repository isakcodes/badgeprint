...
def get_label_context(first_name, last_name, company, default_label_size):
    ...
    context = {
        ...
        "margin_top": float(24) / 100.0,
        "margin_bottom": float(45) / 100.0,
        "margin_left": float(35) / 100.0,
        "margin_right": float(35) / 100.0,
    }
...
def create_label_im_62x29(**kwargs):
    ...
    draw.multiline_text(
        offset, kwargs["name"], (0), font=im_font, align=kwargs["align"]
    )
    ...
    draw.multiline_text(
        company_offset, kwargs["company"], (0), font=company_font, align=kwargs["align"]
    )
    return im
...
def create_label_im_62x100(**kwargs):
    ...
    im_font = ImageFont.truetype(
        kwargs["font_path"], kwargs["font_size"], encoding="utf-8"
    )
    company_font = ImageFont.truetype(kwargs["font_path"], kwargs["company_font_size"])
    im = Image.new("L", (20, 20), "white")
    draw = ImageDraw.Draw(im)
    ...
    if re.search("^type-", kwargs["label_tpl"]):
        if re.search("^type-non-", kwargs["label_tpl"]):
            ...
            if label_tpl != ticket_type:
                ...
                ticket_type_textsize = draw.multiline_textsize(
                    ticket_type, font=ticket_type_font
                )
                ticket_type_vertical_offset = height - ticket_type_textsize[1] - 20
                ticket_type_horizontal_offset = max(
                    (width - ticket_type_textsize[0]) // 2, 0
                )
                ticket_type_offset = (
                    ticket_type_horizontal_offset,
                    ticket_type_vertical_offset,
                )
                draw.multiline_text(
                    ticket_type_offset,
                    ticket_type,
                    (0),
                    font=ticket_type_font,
                    align=kwargs["align"],
                )
                line_height = height - 20 - ticket_type_textsize[1] / 2 + 8
                draw.line(
                    (3, line_height, ticket_type_horizontal_offset - 10, line_height),
                    fill=0,
                    width=3,
                )
                draw.line(
                    (
                        ticket_type_horizontal_offset + ticket_type_textsize[0] + 10,
                        line_height,
                        width - 3,
                        line_height,
                    ),
        ...
        draw.multiline_text(
            eventname_offset, eventname, (0), font=eventname_font, align=kwargs["align"]
        )
