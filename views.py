...
def print_participant_label(request, participant_id):
    if request.user.is_authenticated:
        ...
        try:
            printer = PrinterUser.objects.filter(
                user=request.user, ticket_type=participant.ticket_type
            )[0].printer
        except IndexError:
        ...
        return HttpResponseRedirect(
            reverse("list_event_participant", kwargs={"event_id": participant.event.id})
        )
    else:
        raise Http404("Authentication is required.")
...
def print_participant_label_api(request, participant_id):
    ...
    try:
        printer = PrinterUser.objects.filter(
            user=User(id=1), ticket_type=participant.ticket_type
        )[0].printer
    except IndexError:
